...
for district_name in district_names:
    try:
        map_output = create_district_map(district_name)  # Tạo bản đồ nhiệt
        if map_output:
            # Lưu bản đồ dưới dạng file HTML, thay khoảng trắng bằng dấu gạch dưới để tránh lỗi
            sanitized_name = district_name.replace(" ", "_")
            map_output.save(f"heatmap/{sanitized_name}.html")
            print(f"Đã lưu bản đồ cho quận/huyện: {district_name}")
        else:
            print(f"Không thể tạo bản đồ cho quận/huyện: {district_name}")
    except Exception as e:
        print(f"Lỗi khi xử lý quận/huyện {district_name}: {e}")
